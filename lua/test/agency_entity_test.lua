...
describe("AgencyEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Agency(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Agency(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Agency(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = agency_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"list", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "agency." .. _op, _live and "live" or "unit")
    ...
    local agency_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.agency")))
    local agency_ref01_data = nil
    if #agency_ref01_data_raw > 0 then
      agency_ref01_data = helpers.to_map(agency_ref01_data_raw[1][2])
    end

    -- LIST
    local agency_ref01_ent = client:Agency(nil)
    local agency_ref01_match = {}

    local agency_ref01_list_result, err = agency_ref01_ent:list(agency_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(agency_ref01_list_result)

    -- LOAD
    local agency_ref01_match_dt0 = {
...
function agency_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/agency/AgencyTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read agency test data: " .. entity_data_file)
  end
