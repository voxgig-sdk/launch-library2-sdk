...
describe("DockingEventDirect", function()
  it("should direct-list-docking_event", function()
    local setup = docking_event_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-docking_event", setup.live and "live" or "unit")
  ...
  it("should direct-load-docking_event", function()
    local setup = docking_event_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-docking_event", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
