...
local LauncherEntity = {}
LauncherEntity.__index = LauncherEntity


function LauncherEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LauncherEntity)
  self._name = "launcher"
  self._client = client
...
function LauncherEntity:get_name()
  return self._name
end


function LauncherEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LauncherEntity.new(self._client, opts)
end
...
function LauncherEntity:mark_deleted()
  self._deleted = true
end


function LauncherEntity:deleted()
  return true == self._deleted
end


function LauncherEntity:data_set(args)
  if args ~= nil then
...
function LauncherEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LauncherEntity:match_set(args)
  if args ~= nil then
...
function LauncherEntity:match_get()
...
function LauncherEntity:stream(action, args, callopts)
...
---@param reqmatch LauncherLoadMatch
---@param ctrl? table
---@return Launcher
---@return string? err
function LauncherEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function LauncherEntity:_run_op(ctx, post_done)
  local utility = self._utility
