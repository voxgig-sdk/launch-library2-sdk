-- LaunchLibrary2 SDK ReusableFirstStage entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ReusableFirstStageEntity = {}
ReusableFirstStageEntity.__index = ReusableFirstStageEntity


function ReusableFirstStageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReusableFirstStageEntity)
  self._name = "reusable_first_stage"
  self._client = client
...
function ReusableFirstStageEntity:get_name()
  return self._name
end


function ReusableFirstStageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReusableFirstStageEntity.new(self._client, opts)
end
...
function ReusableFirstStageEntity:mark_deleted()
  self._deleted = true
end


function ReusableFirstStageEntity:deleted()
  return true == self._deleted
end


function ReusableFirstStageEntity:data_set(args)
  if args ~= nil then
...
function ReusableFirstStageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReusableFirstStageEntity:match_set(args)
  if args ~= nil then
...
function ReusableFirstStageEntity:match_get()
...
function ReusableFirstStageEntity:stream(action, args, callopts)
...
function ReusableFirstStageEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ReusableFirstStageEntity
