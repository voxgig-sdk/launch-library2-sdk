-- LaunchLibrary2 SDK Agency entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AgencyEntity = {}
AgencyEntity.__index = AgencyEntity


function AgencyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AgencyEntity)
  self._name = "agency"
  self._client = client
...
function AgencyEntity:get_name()
  return self._name
end


function AgencyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AgencyEntity.new(self._client, opts)
end
...
function AgencyEntity:mark_deleted()
  self._deleted = true
end


function AgencyEntity:deleted()
  return true == self._deleted
end


function AgencyEntity:data_set(args)
  if args ~= nil then
...
function AgencyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AgencyEntity:match_set(args)
  if args ~= nil then
...
function AgencyEntity:match_get()
...
function AgencyEntity:stream(action, args, callopts)
...
function AgencyEntity:_run_op(ctx, post_done)
  local utility = self._utility
