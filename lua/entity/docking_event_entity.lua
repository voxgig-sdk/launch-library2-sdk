...
local DockingEventEntity = {}
DockingEventEntity.__index = DockingEventEntity


function DockingEventEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DockingEventEntity)
  self._name = "docking_event"
  self._client = client
...
function DockingEventEntity:get_name()
  return self._name
end


function DockingEventEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DockingEventEntity.new(self._client, opts)
end
...
function DockingEventEntity:mark_deleted()
  self._deleted = true
end


function DockingEventEntity:deleted()
  return true == self._deleted
end


function DockingEventEntity:data_set(args)
  if args ~= nil then
...
function DockingEventEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DockingEventEntity:match_set(args)
  if args ~= nil then
...
function DockingEventEntity:match_get()
...
function DockingEventEntity:stream(action, args, callopts)
...
---@param reqmatch DockingEventLoadMatch
---@param ctrl? table
---@return DockingEvent
---@return string? err
function DockingEventEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch DockingEventListMatch
---@param ctrl? table
---@return DockingEvent[]
---@return string? err
function DockingEventEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function DockingEventEntity:_run_op(ctx, post_done)
  local utility = self._utility
