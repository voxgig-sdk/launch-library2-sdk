...
local AstronautEntity = {}
AstronautEntity.__index = AstronautEntity


function AstronautEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AstronautEntity)
  self._name = "astronaut"
  self._client = client
...
function AstronautEntity:get_name()
  return self._name
end


function AstronautEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AstronautEntity.new(self._client, opts)
end
...
function AstronautEntity:mark_deleted()
  self._deleted = true
end


function AstronautEntity:deleted()
  return true == self._deleted
end


function AstronautEntity:data_set(args)
  if args ~= nil then
...
function AstronautEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AstronautEntity:match_set(args)
  if args ~= nil then
...
function AstronautEntity:match_get()
...
function AstronautEntity:stream(action, args, callopts)
...
function AstronautEntity:_run_op(ctx, post_done)
  local utility = self._utility
