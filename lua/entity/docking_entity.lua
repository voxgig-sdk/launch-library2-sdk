...
local DockingEntity = {}
DockingEntity.__index = DockingEntity


function DockingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DockingEntity)
  self._name = "docking"
  self._client = client
...
function DockingEntity:get_name()
  return self._name
end


function DockingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DockingEntity.new(self._client, opts)
end
...
function DockingEntity:mark_deleted()
  self._deleted = true
end


function DockingEntity:deleted()
  return true == self._deleted
end


function DockingEntity:data_set(args)
  if args ~= nil then
...
function DockingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DockingEntity:match_set(args)
  if args ~= nil then
...
function DockingEntity:match_get()
...
function DockingEntity:stream(action, args, callopts)
...
function DockingEntity:_run_op(ctx, post_done)
  local utility = self._utility
