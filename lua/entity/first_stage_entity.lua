...
local FirstStageEntity = {}
FirstStageEntity.__index = FirstStageEntity


function FirstStageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FirstStageEntity)
  self._name = "first_stage"
  self._client = client
...
function FirstStageEntity:get_name()
  return self._name
end


function FirstStageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FirstStageEntity.new(self._client, opts)
end
...
function FirstStageEntity:mark_deleted()
  self._deleted = true
end


function FirstStageEntity:deleted()
  return true == self._deleted
end


function FirstStageEntity:data_set(args)
  if args ~= nil then
...
function FirstStageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FirstStageEntity:match_set(args)
  if args ~= nil then
...
function FirstStageEntity:match_get()
...
function FirstStageEntity:stream(action, args, callopts)
...
function FirstStageEntity:_run_op(ctx, post_done)
  local utility = self._utility
