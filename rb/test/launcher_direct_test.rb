...
class LauncherDirectTest < Minitest::Test
  def test_direct_load_launcher
    setup = launcher_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-launcher", setup[:live] ? "live" : "unit")
    if _should_skip
...
def launcher_direct_setup(mockres)
