...
class LauncherEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = launcher_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "launcher." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LAUNCH_LIBRARY2_TEST_LAUNCHER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    launcher_ref01_data = nil
    if launcher_ref01_data_raw.length > 0
      launcher_ref01_data = Helpers.to_map(launcher_ref01_data_raw[0][1])
    end

    # LOAD
    launcher_ref01_ent = client.Launcher(nil)
    ...
    launcher_ref01_data_dt0_load_result = Helpers.to_map(launcher_ref01_data_dt0_loaded.respond_to?(:data_get) ? launcher_ref01_data_dt0_loaded.data_get : launcher_ref01_data_dt0_loaded)
...
def launcher_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "launcher", "LauncherTestData.json")
  ...
  entid_env_raw = ENV["LAUNCH_LIBRARY2_TEST_LAUNCHER_ENTID"]
