...
class DockingEventEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.DockingEvent(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.DockingEvent(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    docking_event_ref01_list_result = docking_event_ref01_ent.list(docking_event_ref01_match, nil)
