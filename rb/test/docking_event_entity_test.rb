...
class DockingEventEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = docking_event_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "docking_event." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    docking_event_ref01_data = nil
    if docking_event_ref01_data_raw.length > 0
      docking_event_ref01_data = Helpers.to_map(docking_event_ref01_data_raw[0][1])
    end

    # LIST
    docking_event_ref01_ent = client.DockingEvent(nil)
    docking_event_ref01_match = {}

    docking_event_ref01_list_result, err = docking_event_ref01_ent.list(docking_event_ref01_match, nil)
    assert_nil err
    assert docking_event_ref01_list_result.is_a?(Array)

    # LOAD
    docking_event_ref01_match_dt0 = {
      "id" => docking_event_ref01_data["id"],
    }
    docking_event_ref01_data_dt0_loaded, err = docking_event_ref01_ent.load(docking_event_ref01_match_dt0, nil)
    assert_nil err
...
def docking_event_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "docking_event", "DockingEventTestData.json")
