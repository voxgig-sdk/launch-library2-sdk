...
class PadEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Pad(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Pad(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = pad_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "pad." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LAUNCH_LIBRARY2_TEST_PAD_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    pad_ref01_data = nil
    if pad_ref01_data_raw.length > 0
    ...
    pad_ref01_data_dt0_load_result = Helpers.to_map(pad_ref01_data_dt0_loaded.respond_to?(:data_get) ? pad_ref01_data_dt0_loaded.data_get : pad_ref01_data_dt0_loaded)
...
def pad_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "pad", "PadTestData.json")
  ...
  entid_env_raw = ENV["LAUNCH_LIBRARY2_TEST_PAD_ENTID"]
