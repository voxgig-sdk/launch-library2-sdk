...
class SpaceStationEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.SpaceStation(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.SpaceStation(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LAUNCH_LIBRARY2_TEST_SPACE_STATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    space_station_ref01_data_dt0_load_result = Helpers.to_map(space_station_ref01_data_dt0_loaded.respond_to?(:data_get) ? space_station_ref01_data_dt0_loaded.data_get : space_station_ref01_data_dt0_loaded)
...
def space_station_basic_setup(extra)
  ...
  entid_env_raw = ENV["LAUNCH_LIBRARY2_TEST_SPACE_STATION_ENTID"]
