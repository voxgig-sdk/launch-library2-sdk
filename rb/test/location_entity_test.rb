...
class LocationEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LAUNCH_LIBRARY2_TEST_LOCATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def location_basic_setup(extra)
  ...
  entid_env_raw = ENV["LAUNCH_LIBRARY2_TEST_LOCATION_ENTID"]
