...
class LaunchVehicleDirectTest < Minitest::Test
  def test_direct_list_launch_vehicle
    setup = launch_vehicle_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-launch_vehicle", setup[:live] ? "live" : "unit")
    if _should_skip
...
def launch_vehicle_direct_setup(mockres)
