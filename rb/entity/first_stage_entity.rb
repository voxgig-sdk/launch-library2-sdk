...
class FirstStageEntity
  ...
  def make
    opts = @_entopts.dup
    FirstStageEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.FirstStage.load works with no args).
  ...
  #
  # @param reqmatch [FirstStageListMatch, Hash, nil] match filter (any subset of
  #   FirstStage fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
