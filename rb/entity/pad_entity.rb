...
class PadEntity
  ...
  # @return [Pad, Hash] the current Pad data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Pad fields)
  def match_get
  ...
  # Load a single Pad.
  #
  # @param reqmatch [PadLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Pad.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Pad, Hash] the loaded Pad; raises LaunchLibrary2Error on failure
  ...
  #
  # @param reqmatch [PadListMatch, Hash, nil] match filter (any subset of
  #   Pad fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Pad>, Array] the matching Pad items; raises LaunchLibrary2Error on failure
