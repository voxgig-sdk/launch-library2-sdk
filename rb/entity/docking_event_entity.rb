...
class DockingEventEntity
  ...
  def make
    opts = @_entopts.dup
    DockingEventEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = LaunchLibrary2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [DockingEvent, Hash] the current DockingEvent data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = LaunchLibrary2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of DockingEvent fields)
  def match_get
  ...
  #
  # @param reqmatch [DockingEventLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.DockingEvent.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [DockingEvent, Hash] the loaded DockingEvent; raises LaunchLibrary2Error on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = LaunchLibrary2Helpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [DockingEventListMatch, Hash, nil] match filter (any subset of
  #   DockingEvent fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<DockingEvent>, Array] the matching DockingEvent items; raises LaunchLibrary2Error on failure
