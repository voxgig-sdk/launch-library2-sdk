...
class AstronautEntity
  ...
  def make
    opts = @_entopts.dup
    AstronautEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Astronaut.load works with no args).
  ...
  #
  # @param reqmatch [AstronautListMatch, Hash, nil] match filter (any subset of
  #   Astronaut fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
