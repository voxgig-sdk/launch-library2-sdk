...
func TestPadEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LAUNCHLIBRARY__TEST_PAD_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		padRef01DataDt0LoadResult := core.ToMapAny(padRef01DataDt0Loaded)
...
func padBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LAUNCHLIBRARY__TEST_PAD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LAUNCHLIBRARY__TEST_PAD_ENTID"])
