...
func TestLocationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LAUNCH_LIBRARY2_TEST_LOCATION_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func locationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LAUNCH_LIBRARY2_TEST_LOCATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LAUNCH_LIBRARY2_TEST_LOCATION_ENTID"])
