...
func TestSpaceStationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LAUNCHLIBRARY__TEST_SPACE_STATION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		spaceStationRef01DataDt0LoadResult := core.ToMapAny(spaceStationRef01DataDt0Loaded)
...
func space_stationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LAUNCHLIBRARY__TEST_SPACE_STATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LAUNCHLIBRARY__TEST_SPACE_STATION_ENTID"])
