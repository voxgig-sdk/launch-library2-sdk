...
func TestAgencyEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LAUNCHLIBRARY__TEST_AGENCY_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		agencyRef01DataDt0LoadResult := core.ToMapAny(agencyRef01DataDt0Loaded)
...
func agencyBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LAUNCHLIBRARY__TEST_AGENCY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LAUNCHLIBRARY__TEST_AGENCY_ENTID"])
