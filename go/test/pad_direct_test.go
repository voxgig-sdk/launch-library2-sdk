...
func TestPadDirect(t *testing.T) {
	t.Run("direct-list-pad", func(t *testing.T) {
		setup := padDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-pad", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-pad", func(t *testing.T) {
		setup := padDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-pad", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type padDirectSetupResult struct {
...
func padDirectSetup(mockres any) *padDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"LAUNCH_LIBRARY2_TEST_PAD_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["LAUNCH_LIBRARY2_TEST_PAD_ENTID"]; ok {
		...
		return &padDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &padDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
