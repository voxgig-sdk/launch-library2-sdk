...
[MCP](https://modelcontextprotocol.io) server exposing the LaunchLibrary2 SDK as
two agent tools — `launch-library2_list` and `launch-library2_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/launch-library2-mcp)
make build

# 2. Provide credentials via the environment
export LAUNCH_LIBRARY2_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user launch-library2 \
  -- /absolute/path/to/launch-library2-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./launch-library2-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// launch-library2_list: first page of records
{ "entity": "agency" }
{ "entity": "agency", "query": { } }

// launch-library2_load: one record by id
{ "entity": "agency", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/launch-library2-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user launch-library2 \
     -- "$PWD"/dist/*/launch-library2-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `launch-library2_list` and `launch-library2_load` tools now appear
   in new sessions. Ask the agent to *"list agency using launch-library2"*
   and it calls `launch-library2_list` with `{"entity":"agency"}`.
...
```sh
export LAUNCH_LIBRARY2_APIKEY=sk_live_xxx            # API key
export LAUNCH_LIBRARY2_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `launch-library2_list` tool
...
### Call the `launch-library2_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `launch-library2_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `launch-library2_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Field | Type | Notes |
|-------|------|-------|
| `entity` | string | One of the 15 supported entities (see below). |
...
| Variable | Purpose |
|----------|---------|
| `LAUNCH_LIBRARY2_APIKEY` | API key sent with every request. |
| `LAUNCH_LIBRARY2_BASE` | Optional override of the API base URL. |

### Entities

The 15 entities valid as the `entity` argument:
...
```sh
./launch-library2-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"launch-library2_load","arguments":{"entity":"agency","query":{"id":1}}}}'
```
