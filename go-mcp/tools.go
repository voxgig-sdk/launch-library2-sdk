...
type Args struct {
	Entity string         `json:"entity" jsonschema:"agency | astronaut | docking | docking_event | event | expedition | first_stage | launch | launch_vehicle | launcher | location | pad | reusable_first_stage | space_station | spacecraft"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.LaunchLibrary2SDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "launch-library2_list",
		Description: "List records from LaunchLibrary2. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "launch-library2_load",
		Description: "Load a single record from LaunchLibrary2. " +
...
func runOp(client *sdk.LaunchLibrary2SDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.LaunchLibrary2SDK, name string) (sdk.LaunchLibrary2Entity, error) {
	switch strings.ToLower(name) {
	case "agency":
		return client.Agency(nil), nil
	case "astronaut":
		return client.Astronaut(nil), nil
	case "docking":
		return client.Docking(nil), nil
	case "docking_event":
		return client.DockingEvent(nil), nil
	case "event":
		return client.Event(nil), nil
	case "expedition":
		return client.Expedition(nil), nil
	case "first_stage":
		return client.FirstStage(nil), nil
	case "launch":
		return client.Launch(nil), nil
	case "launch_vehicle":
		return client.LaunchVehicle(nil), nil
	case "launcher":
		return client.Launcher(nil), nil
	case "location":
		return client.Location(nil), nil
	case "pad":
		return client.Pad(nil), nil
	case "reusable_first_stage":
		return client.ReusableFirstStage(nil), nil
	case "space_station":
		return client.SpaceStation(nil), nil
	case "spacecraft":
