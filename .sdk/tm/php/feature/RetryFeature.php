...
// LaunchLibrary2 SDK retry feature
...
class LaunchLibrary2RetryFeature extends LaunchLibrary2BaseFeature
{
    ...
    public function init(LaunchLibrary2Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (LaunchLibrary2Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(LaunchLibrary2Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(LaunchLibrary2Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
