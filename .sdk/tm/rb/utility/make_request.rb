...
module LaunchLibrary2Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = LaunchLibrary2Response.new({})
    ...
    if fetch_err
      ...
      response = LaunchLibrary2Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = LaunchLibrary2Response.new(fetched)
    else
