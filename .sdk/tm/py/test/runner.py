# LaunchLibrary2 SDK test runner
...
class LaunchLibrary2TestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            LaunchLibrary2TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = LaunchLibrary2TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = LaunchLibrary2TestRunner.getenv("PROJECTNAME_TEST_LIVE")
        override = LaunchLibrary2TestRunner.getenv("PROJECTNAME_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = LaunchLibrary2TestRunner.getenv(key)
        ...
        explain = LaunchLibrary2TestRunner.getenv("PROJECTNAME_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if LaunchLibrary2TestRunner._test_control is not None:
            return LaunchLibrary2TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                LaunchLibrary2TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            LaunchLibrary2TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = LaunchLibrary2TestRunner.load_test_control()
...
def load_env_local():
    LaunchLibrary2TestRunner.load_env_local()
...
def entity_list_to_data(lst):
    return LaunchLibrary2TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return LaunchLibrary2TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return LaunchLibrary2TestRunner.load_test_control()


def live_delay_ms():
    return LaunchLibrary2TestRunner.live_delay_ms()
