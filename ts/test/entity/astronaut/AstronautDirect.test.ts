...
describe('AstronautDirect', async () => {
  ...
  test('direct-load-astronaut', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-astronaut', setup.live)) return
  ...
  test('direct-list-astronaut', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-astronaut', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LAUNCH_LIBRARY2_TEST_ASTRONAUT_ENTID']
