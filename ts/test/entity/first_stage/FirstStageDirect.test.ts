...
describe('FirstStageDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new LaunchLibrary2SDK({
  ...
  test('direct-load-first_stage', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-first_stage', setup.live)) return
  ...
  test('direct-list-first_stage', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-first_stage', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LAUNCH_LIBRARY2_TEST_FIRST_STAGE_ENTID']
