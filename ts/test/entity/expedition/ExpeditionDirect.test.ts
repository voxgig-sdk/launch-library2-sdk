...
describe('ExpeditionDirect', async () => {
  ...
  test('direct-load-expedition', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-expedition', setup.live)) return
  ...
  test('direct-list-expedition', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-expedition', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LAUNCH_LIBRARY2_TEST_EXPEDITION_ENTID']
