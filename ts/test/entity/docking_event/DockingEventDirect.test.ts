...
describe('DockingEventDirect', async () => {
  ...
  test('direct-load-docking_event', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-docking_event', setup.live)) return
  ...
  test('direct-list-docking_event', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-docking_event', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LAUNCH_LIBRARY2_TEST_DOCKING_EVENT_ENTID']
