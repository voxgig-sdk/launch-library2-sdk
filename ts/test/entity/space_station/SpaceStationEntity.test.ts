...
describe('SpaceStationEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LAUNCH_LIBRARY__TEST_SPACE_STATION_ENTID JSON to run live')
      return
    }
    ...
    const space_station_ref01_list = await space_station_ref01_ent.list(space_station_ref01_match)


    // LOAD
    const space_station_ref01_match_dt0: any = {}
    space_station_ref01_match_dt0.id = space_station_ref01_data.id
    const space_station_ref01_data_dt0 = await space_station_ref01_ent.load(space_station_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['LAUNCH_LIBRARY__TEST_SPACE_STATION_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'LAUNCH_LIBRARY__TEST_SPACE_STATION_ENTID': idmap,
    'LAUNCH_LIBRARY__TEST_LIVE': 'FALSE',
    'LAUNCH_LIBRARY__TEST_EXPLAIN': 'FALSE',
    'LAUNCH_LIBRARY__APIKEY': 'NONE',
  })

  idmap = env['LAUNCH_LIBRARY__TEST_SPACE_STATION_ENTID']
