// Command LaunchLibrary2-cli is a CLI + REPL for the LaunchLibrary2 SDK,
...
const prompt = "launch-library2"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "agency astronaut docking docking_event event expedition first_stage launch launch_vehicle launcher location pad reusable_first_stage space_station spacecraft"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      LAUNCH_LIBRARY2_APIKEY (api key), LAUNCH_LIBRARY2_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: LAUNCH_LIBRARY2_APIKEY carries the API key and
	// LAUNCH_LIBRARY2_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("LAUNCH_LIBRARY2_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("LAUNCH_LIBRARY2_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewLaunchLibrary2SDK(opts)
