...
boru-driven command-line client **and** interactive REPL for the LaunchLibrary2
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/launch-library2-cli)
make build

# 2. See usage (words, entities, env vars)
./launch-library2-cli --help
...
./launch-library2-cli load 1 agency            # {id:1} shorthand
./launch-library2-cli load '{id:1}' agency       # explicit match map
./launch-library2-cli list astronaut

# 5. Override the API base URL for a single call
LAUNCH_LIBRARY2_BASE=https://api.example.com ./launch-library2-cli list agency

# 6. No arguments -> interactive REPL
./launch-library2-cli
launch-library2> list agency
launch-library2> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/launch-library2-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/launch-library2-cli list agency
   ```
...
```sh
./launch-library2-cli load 1 agency          # scalar shorthand for {id:1}
./launch-library2-cli load '{id:1}' agency     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `launch-library2>`). Each line is
evaluated as its own boru expression:

```text
$ ./launch-library2-cli
launch-library2> list agency
launch-library2> /help
launch-library2> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `agency`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/launch-library2-cli`. |
...
The 15 entities this SDK exposes (any is valid as `<entity>`):
...
(`./launch-library2-cli <expr>`) and interactively (the REPL), and expressions compose the
