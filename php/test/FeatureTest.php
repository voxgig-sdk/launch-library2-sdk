...
// LaunchLibrary2 SDK feature test
//
...
class FtCtrl extends LaunchLibrary2Control
{
...
class FtClient
{
    ...
    private LaunchLibrary2Utility $utility;

    public function __construct(LaunchLibrary2Utility $utility, array $options)
    {
    ...
    public function get_utility(): LaunchLibrary2Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (LaunchLibrary2Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public LaunchLibrary2Context $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (LaunchLibrary2Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = LaunchLibrary2Config::make_feature($name);
    ...
    private static function buildUrl(LaunchLibrary2Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof LaunchLibrary2Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new LaunchLibrary2Response($fetched);
        }
    ...
    private function fail(LaunchLibrary2Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (LaunchLibrary2Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
