...
class LaunchEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LAUNCHLIBRARY__TEST_LAUNCH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function launch_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("LAUNCHLIBRARY__TEST_LAUNCH_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "LAUNCHLIBRARY__TEST_LAUNCH_ENTID" => $idmap,
        "LAUNCHLIBRARY__TEST_LIVE" => "FALSE",
        "LAUNCHLIBRARY__TEST_EXPLAIN" => "FALSE",
    ]);

    $idmap_resolved = Helpers::to_map(
        $env["LAUNCHLIBRARY__TEST_LAUNCH_ENTID"]);
