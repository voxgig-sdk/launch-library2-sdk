...
class ExpeditionEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = LaunchLibrary2SDK::test(null, null);
        $ent = $testsdk->Expedition(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Expedition(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Expedition(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = expedition_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "expedition." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $expedition_ref01_data = null;
        if (count($expedition_ref01_data_raw) > 0) {
...
function expedition_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/expedition/ExpeditionTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["expedition01", "expedition02", "expedition03"] as $k) {
