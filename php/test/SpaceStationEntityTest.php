...
class SpaceStationEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = LaunchLibrary2SDK::test(null, null);
        $ent = $testsdk->SpaceStation(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->SpaceStation(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->SpaceStation(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = space_station_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "space_station." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LAUNCH_LIBRARY2_TEST_SPACE_STATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $space_station_ref01_data = null;
        if (count($space_station_ref01_data_raw) > 0) {
        ...
        $space_station_ref01_data_dt0_load_result = Helpers::to_map(is_object($space_station_ref01_data_dt0_loaded) && method_exists($space_station_ref01_data_dt0_loaded, 'data_get') ? $space_station_ref01_data_dt0_loaded->data_get() : $space_station_ref01_data_dt0_loaded);
...
function space_station_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/space_station/SpaceStationTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["space_station01", "space_station02", "space_station03"] as $k) {
    ...
    $entid_env_raw = getenv("LAUNCH_LIBRARY2_TEST_SPACE_STATION_ENTID");
