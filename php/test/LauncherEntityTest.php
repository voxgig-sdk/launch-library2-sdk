...
class LauncherEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LAUNCHLIBRARY__TEST_LAUNCHER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        [$launcher_ref01_data_dt0_loaded, $err] = $launcher_ref01_ent->load($launcher_ref01_match_dt0, null);
        $this->assertNull($err);
...
function launcher_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("LAUNCHLIBRARY__TEST_LAUNCHER_ENTID");
