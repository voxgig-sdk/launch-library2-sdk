...
class ReusableFirstStageEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LAUNCH_LIBRARY2_TEST_REUSABLE_FIRST_STAGE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function reusable_first_stage_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("LAUNCH_LIBRARY2_TEST_REUSABLE_FIRST_STAGE_ENTID");
