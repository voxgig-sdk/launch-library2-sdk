...
The PHP SDK for the LaunchLibrary2 API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Agency(null)->list(null, null);
...
```php
[$result, $err] = $client->Agency(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->LaunchLibrary2(null)->load(
    ["id" => "test01"], null
);
