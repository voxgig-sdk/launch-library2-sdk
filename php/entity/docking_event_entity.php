...
// LaunchLibrary2 SDK DockingEvent entity
...
class DockingEventEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new DockingEventEntity($this->_client, $opts);
    }

    /**
     * @param DockingEvent|array $args DockingEvent data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = LaunchLibrary2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return DockingEvent|array The current DockingEvent data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of DockingEvent fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = LaunchLibrary2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of DockingEvent fields).
     */
    ...
    /**
     * Load a single DockingEvent.
     *
     * @param DockingEventLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed DockingEventLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return DockingEvent|array The loaded DockingEvent as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = LaunchLibrary2Helpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List DockingEvent items matching the given filter.
     *
     * @param DockingEventListMatch|array|null $reqmatch Match filter (any subset
     *   of DockingEvent fields) as an assoc-array; DockingEventListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return DockingEvent[]|array A list of DockingEvent items as assoc-arrays at
