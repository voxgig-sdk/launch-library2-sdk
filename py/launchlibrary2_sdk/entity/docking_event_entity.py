...
from launchlibrary2_sdk.launchlibrary2_types import (
    DockingEvent,
...
class DockingEventEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DockingEventEntity(self._client, opts)
    ...
    def data_get(self) -> DockingEvent:
    ...
    def match_get(self) -> DockingEvent:
    ...
    def load(self, reqmatch=None, ctrl=None) -> DockingEvent:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.DockingEvent().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[DockingEvent]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.DockingEvent().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
