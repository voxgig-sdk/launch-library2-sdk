...
class TestLauncherDirect:

    def test_should_direct_load_launcher(self):
        setup = _launcher_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-launcher", "live" if setup["live"] else "unit")
        if _skip:
...
def _launcher_direct_setup(mockres):
