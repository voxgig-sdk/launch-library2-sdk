    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LAUNCH_LIBRARY2_TEST_LOCATION_ENTID JSON to run live")
        client = setup["client"]
...
def _location_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "LAUNCH_LIBRARY2_TEST_LOCATION_ENTID")
    _idmap_overridden = _entid_env_raw is not None and _entid_env_raw.strip().startswith("{")

    env = runner.env_override({
        "LAUNCH_LIBRARY2_TEST_LOCATION_ENTID": idmap,
        "LAUNCH_LIBRARY2_TEST_LIVE": "FALSE",
        "LAUNCH_LIBRARY2_TEST_EXPLAIN": "FALSE",
    })

    idmap_resolved = helpers.to_map(
        env.get("LAUNCH_LIBRARY2_TEST_LOCATION_ENTID"))
