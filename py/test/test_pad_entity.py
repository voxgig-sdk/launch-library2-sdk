...
class TestPadEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Pad(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Pad(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LAUNCH_LIBRARY2_TEST_PAD_ENTID JSON to run live")
        client = setup["client"]
        ...
        pad_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(pad_ref01_data_dt0_loaded))
...
def _pad_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("LAUNCH_LIBRARY2_TEST_PAD_ENTID"))
