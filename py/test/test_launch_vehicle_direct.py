...
class TestLaunchVehicleDirect:

    def test_should_direct_list_launch_vehicle(self):
        setup = _launch_vehicle_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-launch_vehicle", "live" if setup["live"] else "unit")
        if _skip:
...
def _launch_vehicle_direct_setup(mockres):
