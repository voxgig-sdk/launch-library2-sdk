...
class TestFirstStageEntity:

    def test_should_create_instance(self):
        testsdk = LaunchLibrary2SDK.test(None, None)
        ent = testsdk.FirstStage(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.FirstStage(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.FirstStage(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _first_stage_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "first_stage." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LAUNCH_LIBRARY2_TEST_FIRST_STAGE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        first_stage_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.first_stage")))
        first_stage_ref01_data = None
        if len(first_stage_ref01_data_raw) > 0:
        ...
        first_stage_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(first_stage_ref01_data_dt0_loaded))
...
def _first_stage_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/first_stage/FirstStageTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "LAUNCH_LIBRARY2_TEST_FIRST_STAGE_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("LAUNCH_LIBRARY2_TEST_FIRST_STAGE_ENTID"))
