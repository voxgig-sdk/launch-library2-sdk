...
class TestLauncherEntity:

    def test_should_create_instance(self):
        testsdk = LaunchLibrary2SDK.test(None, None)
        ent = testsdk.Launcher(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _launcher_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "launcher." + _op, "live" if _live else "unit")
            if _skip:
        ...
        launcher_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(launcher_ref01_data_dt0_loaded))
...
def _launcher_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/launcher/LauncherTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("LAUNCH_LIBRARY2_TEST_LAUNCHER_ENTID"))
